    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(
          `https://backend.j-byu.shop/api/products/byCatgId/${KIDS_CATEGORY_ID}`
        );
        ...
        if (err.response?.status === 404) {
          setKidsProducts([]); // Handle 404 gracefully
          return;
        }
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    return (
      <Pressable
        ...
        _pressed={{ opacity: 0.7 }} // Visual feedback on press
      >
        <Image
          source={{ uri: imageUrl }}
          alt={item.title}
    ...
    <View
      ...
      <HStack
        ...
        <Text
          ...
          <Text style={{ color: "#F7CF9D" }}>{t("New")}</Text> {/* Assuming "اخر" was meant to be "New" */}
        </Text>
        <Pressable
          px={4}
          py={2}
